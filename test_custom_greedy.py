...
def main():
    ...
    if not args.no_vis:
        ...
        image = ax.imshow(obs)
        ax.axis("off")
        ax.set_title("Initial Board  --  Greedy solving...")
        fig.canvas.draw()
    ...
    if plan is None:
        ...
        if not args.no_vis:
            ax.set_title("Greedy: no solution found")
            fig.canvas.draw()
            fig.canvas.flush_events()
            plt.pause(args.hold)
            plt.ioff()
    ...
    print(f"Greedy found a solution. {len(plan)} steps, {agent.pushes} pushes.")

    if not args.no_vis:
        title = f"Greedy ready  --  {len(plan)} steps  [Space / -> to begin]" if args.manual \
                else f"Greedy: solution found  --  {len(plan)} steps, {agent.pushes} pushes"
        ax.set_title(title)
        fig.canvas.draw()
        fig.canvas.flush_events()
