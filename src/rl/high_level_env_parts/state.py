...
def sum_min_goal_distance(box_positions, goal_positions):
    """
    Greedily match boxes to the nearest remaining goals by Manhattan distance
    """
    remaining_goals = sorted(list(goal_positions))
    total_distance = 0

    for box_position in sorted(list(box_positions)):
        if not remaining_goals:
            break
        distances = [abs(box_position[0] - goal_position[0]) + abs(box_position[1] - goal_position[1]) for goal_position in remaining_goals]
